...
class main_model(nn.Module):
    ...
    def forward(self, imgs):
        ...
        ######  Global Branch ######
        x_s, H, W = self.patch_embed(imgs) # W:56 H:56 x_s:(BX3136X96)
        x_s = self.pos_drop(x_s) # x_s:(BX3136X96)
        x_s_1, H, W = self.layers1(x_s, H, W)  # W:56 H:656 x_s_1:(BX3136X96)
        x_s_2, H, W = self.layers2(x_s_1, H, W)  # W:28 H:28 x_s:(BX784X192)
        x_s_3, H, W = self.layers3(x_s_2, H, W)  # W:16 H:16 x_s:(BX196X384)
        x_s_4, H, W = self.layers4(x_s_3, H, W)  # W:8 H:8 x_s:(BX49X768)

        # [B,L,C] ---> [B,C,H,W]
        x_s_1 = torch.transpose(x_s_1, 1, 2)  # (BX96X3136)
        x_s_1 = x_s_1.view(x_s_1.shape[0], -1, 56, 56)  # (BX96X56X56)
        x_s_2 = torch.transpose(x_s_2, 1, 2)
        x_s_2 = x_s_2.view(x_s_2.shape[0], -1, 28, 28)  # (BX192X28X28)
        x_s_3 = torch.transpose(x_s_3, 1, 2)
        x_s_3 = x_s_3.view(x_s_3.shape[0], -1, 14, 14)  # (BX384X14X14)
        x_s_4 = torch.transpose(x_s_4, 1, 2)
        x_s_4 = x_s_4.view(x_s_4.shape[0], -1, 7, 7)  # (BX768X7X7)

        ######  Local Branch ######
        x_c = self.downsample_layers[0](imgs)  # (BX96X56X56)
        x_c_1 = self.stages[0](x_c)  # (BX96X56X56)
        x_c = self.downsample_layers[1](x_c_1)  # (BX192X28X28)
        x_c_2 = self.stages[1](x_c)  # (BX192X28X28)
        x_c = self.downsample_layers[2](x_c_2)  # (BX384X14X14)
        x_c_3 = self.stages[2](x_c)  # (BX384X14X14)
        x_c = self.downsample_layers[3](x_c_3)  # (BX768X7X7)
        x_c_4 = self.stages[3](x_c)  # (BX768X7X7)

        ###### Hierachical Feature Fusion Path ######
        x_f_1 = self.fu1(x_c_1, x_s_1, None)  # (BX96X56X56)
        x_f_2 = self.fu2(x_c_2, x_s_2, x_f_1)  # (BX192X28X28)
        x_f_3 = self.fu3(x_c_3, x_s_3, x_f_2)  # (BX384X14X14)
        x_f_4 = self.fu4(x_c_4, x_s_4, x_f_3)  # (BX768X7X7)
        x_fu = self.conv_norm(x_f_4.mean([-2, -1]))  # (BX768)  # global average pooling, (N, C, H, W) -> (N, C)
        x_fu = self.conv_head(x_fu) # (BX1560)
