...
if __name__ == '__main__':
    ...
    for folder in sub_folders:
        root_path = os.path.join(input_path, folder)
        out_root_path = os.path.join(output_path, folder)

        if not os.path.exists(out_root_path):
            os.mkdir(out_root_path)

        jpg_list = glob.glob(os.path.join(os.path.join(root_path, '*jpg')))

        for jpg_file in jpg_list:
            mat_file = jpg_file.replace('.jpg', '.mat')

            run_posmap_300W_LP(bfm, jpg_file, mat_file, out_root_path)

            print(jpg_file, mat_file)
