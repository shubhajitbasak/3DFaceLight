...
def take(num_step, data_loader):
    num_epoch = num_step // len(data_loader.dataset) + 1
...
def load_dataset(cfg, shuffle=False, nun_workers=8):
    ...
    data_loader = DataLoader(dataset, batch_size=cfg['batch_size'],
                             drop_last=True, shuffle=shuffle, num_workers=nun_workers)
    print("[*] Load Success")
    return data_loader
...
class PRNet_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data['Posmap'] = np.load(self.datas_list[index])
