...
from GraphTest.params import Args
    ...
    def __init__(self, layers, K=20):
        ...
        if layers is None:
            ...
            for i in range(len(self.layers) - 1):
                if i == 0:
                    mlp_layers['conv_bn_block_{}'.format(i + 1)] = conv_bn_block(2 * self.layers[i], self.layers[i + 1],
                                                                                 1)
                else:
                    mlp_layers['conv_bn_block_{}'.format(i + 1)] = conv_bn_block(self.layers[i], self.layers[i + 1], 1)
            self.mlp = nn.Sequential(mlp_layers).to(Args.device)
