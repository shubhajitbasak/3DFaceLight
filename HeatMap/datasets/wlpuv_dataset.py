...
class wlpuvDatasets(data.Dataset):
    """Represents a dense position map dataset (e.g. - 300WLP).
    ...
    def data_aug(self, img, pos):
        ...
        new_img = tf.warp(img, (tf_shift + (tform + tf_shift_inv)).inverse)
        ...
        # aug posmap
        # pos  # 256,256,3
        vertices = np.reshape(pos, [-1, 3]).T  # 3,65536
        ...
        new_posmap = np.reshape(vertices.T, [self.posmap_size, self.posmap_size, 3])  # 256,256,3
        ...
        return new_img, new_posmap

    def __getitem__(self, index):

        img = cv2.imread(self.datas_list[index].replace('.npy', '.jpg'))
        pos = np.load(self.datas_list[index])

        if self.is_aug:
            img_aug, pos_aug = self.data_aug(img, pos)

        transform = transforms.ToTensor()

        img_aug = transform((img_aug / 255.).astype(np.float32))
        pos_aug = (pos_aug / 255.).astype(np.float32)
        kpts = get_landmarks(pos_aug, self.uv_kpt_ind)
        verts = get_vertices(pos_aug, self.resolution_op, self.face_ind)
        verts_filtered = verts[self.filtered_indexs]
        # data['kpt_filtered'] = vert[self.filtered_kpt]
        # kpts_filtered = verts_filtered[self.filtered_kpt_500]

        data = {'Image': img_aug,
                # 'Posmap': pos_aug,
                'kpt': kpts,
                # 'kpt_filtered': kpts_filtered,
                'vertices_filtered': verts_filtered}
                # 'kpt_gt': kpt_gt}
